 ...
 *
*/
#define HR_CALC_LIMIT		6		// Max number of recorded beats to count in calculation
#define HR_LOG_LEN	128
int hrLog[HR_LOG_LEN] = { 0, };
int hrLogNext = 0;

#define HR_LOG_STATE_IDLE	0
#define HR_LOG_STATE_DETECT	1
int hrLogState = HR_LOG_STATE_IDLE;
...
#define HR_LOG_DELAY	(2)
int hrLogDelay = 0;

#define HR_LOG_CHANGE_LOOPS	1
	...
	else
	{
		...
		else
		{
			...
			for ( i = 0 ; i < HR_CALC_LIMIT ; i++ )
			{
				...
				{
					firstTime = hrLog[prev]; // Recorded start of the first breath
					beats += 1;
					intervals += 1;
					prev -= 1;
					if ( prev < 0 ) 
		...
		if ( intervals > 0 )
		{
			...
			{
				avg_rate = ( ( ( (float)intervals / (float)totalTime ) ) * 60000 );
			}
			if ( avg_rate < 0 )
			{
				avg_rate = 0;
			}
			else if ( avg_rate > 300 )
			{
				avg_rate = 300;
			}
		}
